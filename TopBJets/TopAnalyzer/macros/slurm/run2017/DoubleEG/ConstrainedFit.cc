...
int ConstrainedFit::Fit(int niter) {
  ...
  if(_verbose >= 2) {
    ...
    TVectorD par1 = _xx.at(0); TVectorD par2 = _xx.at(1); par1.Print(); par2.Print(); 
    if(_xx.size() == 3) {TVectorD par3 = _xx.at(2); par3.Print();}
    TMatrixD cov1 = _vv.at(0); TMatrixD cov2 = _vv.at(1); cov1.Print(); cov2.Print();
    if(_xx.size() == 3) {TMatrixD cov3 = _vv.at(2); cov3.Print();}
  }
  ...
  do {
    ...
    //g.Print();
    d = -1.0*bwb*bt*w*g; // (6x6 * 6x9 * 9x9 * 9x1)
    e = v*a*w*(g+b*d); //
    //d.Print(); e.Print();
    for(int i=0; i<NPar(); i++) {
      _par[i] += d[i]; // Check
